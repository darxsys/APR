...
Matrix Matrix::invert() {
	Matrix p(*this);
	int status = this->LUPdecompose(p);
	if (status == 0) {
		cout << "Error in LUP decomposition." << endl;
		exit(2);
	}

	if (fabs((*this)[(height - 1) * width + width - 1]) < EPSILON) {
		cout << "ERROR: Matrix A has 0 as its last element. Not possible to solve." << endl;
		exit(1);
	}

	Matrix b(this->height, 1);
	Matrix inverse(this->height, this->width);

	for (int i = 0; i < this->height; i++) {
		// this->initializePermutationMatrix(b);
		...
		this->forwardSupstitution(b);
		this->backwardSupstitution(b);

		for (int j = 0; j < this->height; j++) {
			inverse[i * this->width + j] = b[j];
		}
	}
